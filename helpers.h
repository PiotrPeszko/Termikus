...
struct  strDateTime // structure holds current date & time
{
	byte hour;
	byte minute;
	byte second;
...
uint16_t epochDays;         // stores number of full days since 1970
uint32_t epochMinutes;      // stores number of full minutes since 1970 to now 
uint32_t epochSTARTminuty;  // stores number of full minutes since 1970 to programm start
uint32_t epochSTOPminuty;   // stores number of full minutes since 1970 to programm stop
...
void ConvertUnixTimeStamp( unsigned long TimeStamp, struct strDateTime* DateTime)
{
    ...
    epochDays = time;  // zapisanie ilości dni do globalnej zmiennej
